...
const getPizzaList = () => {
  fetch('/api/pizzas') //get all pizzas
  .then(response => response.json()) //transform into json
  .then(pizzaListArr => {
    pizzaListArr.forEach(printPizza); //run each one through printPizza function using forEach() method
  })
  .catch(err => {
    console.log(err);
  });
